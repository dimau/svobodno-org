...
/*************************************************************************************
 * Команда от админа на внесение контакта в список собственников
 ************************************************************************************/

if ($action == "markAsOwnerAdvert") {

    // Только админ может пометить объявление в качестве объявления от собственника в форме редактирования
    // Это удобно при первичном редактировании и публикации оператором автоматически распарсенного объявления из чужой базы
    // Кроме того, проверяем, что контактный телефон указан в объявлении, в противном случае ничего не делаем
    if (($isAdmin['newAdvertAlien'] || $isAdmin['searchUser']) && $property->getContactTelephonNumber() != "") {

        // Внесем телефонный номер в БД как от собственника, либо изменим статус уже имеющейся записи про данный телефонный номер на "собственник"
        if (count(DBconnect::selectKnownPhoneNumber($property->getContactTelephonNumber())) == 0) {
            // Такого телефонного номера еще нет в БД
            DBconnect::insertKnownPhoneNumber(array("phoneNumber" => $property->getContactTelephonNumber(), "status" => "собственник", "dateOfLastPublication" => time()));
        } else {
            // Такой телефонный номер уже есть в БД
            DBconnect::updateKnownPhoneNumberStatus($property->getContactTelephonNumber(), "собственник");
            DBconnect::updateKnownPhoneNumberDate($property->getContactTelephonNumber(), time());
        }
    }
}
