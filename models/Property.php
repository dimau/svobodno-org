...
class Property {
    ...
    public function getContactTelephonNumber() {
     ...
     *  2. Очистить дату въезда и выезда у объекта
     *  3. Удалить все уведомления (типа Новый подходящий объект)
     *  4. Если объявление имеет полноту = 0, то оно переносится в архив
    ...
    public function unpublishAdvert() {
        ...
        if ($this->id == "" || $this->status == "" || $this->completeness == "") return array("Не удалось снять с публикации объявление: недостаточно данных. Попробуйте повторить немного позже или свяжитесь с нами: 8-922-179-59-07");
        ...
        if (!$this->saveCharacteristicToDB("edit")) return array("Не удалось снять с публикации объявление: ошибка доступа к базе. Попробуйте повторить немного позже или свяжитесь с нами по телефону 8-922-179-59-07");
        ...
        if ($this->completeness == "0") {
            if (!DBconnect::insertPropertyCharacteristicToArchive($this->getCharacteristicData()) || !DBconnect::deletePropertyCharacteristicForId($this->id)) {
                return array("Не удалось снять с публикации объявление: ошибка доступа к базе. Попробуйте повторить немного позже или свяжитесь с нами по телефону 8-922-179-59-07");
            }
        }
    ...
    public function publishAdvert() {
        ...
        // Валидация начальных данных
        if ($this->id == "" || $this->status == "" || $this->completeness == "") return array("Не удалось опубликовать объявление: недостаточно данных. Попробуйте повторить немного позже или обратитесь к нам: 8-922-179-59-07");
        ...
        if (!$this->saveCharacteristicToDB("edit")) return array("Не удалось опубликовать объявление: ошибка при записи данных в базу. Попробуйте повторить немного позже или обратитесь к нам: 8-922-179-59-07");
