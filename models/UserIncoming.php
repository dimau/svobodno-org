...
class UserIncoming extends User
{
	...
	public function login() {
		...
		$stmt = DBconnect::get()->stmt_init();
		if (($stmt->prepare("SELECT id, typeTenant, typeOwner, typeAdmin, name, secondName, surname, telephon, login, password, user_hash, favoritePropertiesId FROM users WHERE user_hash=? OR login=?") === FALSE)
		...
		if ($user_hashFromDB == $sessionId || md5($loginFromDB . $passwordFromDB) == $_COOKIE['password']) {
			...
			if (isset($res[0]['favoritePropertiesId'])) {
				if (($unserializedData = unserialize($res[0]['favoritePropertiesId'])) != FALSE && is_array($unserializedData)) $this->favoritePropertiesId = $unserializedData;
			}
