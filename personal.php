...
if ($action == "publishAdvert" && $propertyId != "" && $propertyId != 0) {
    ...
    if ($collectionProperty->hasPropertyId($propertyId)) {
        ...
        if ($property->readCharacteristicFromDB() && $property->readFotoInformationFromDB()) {
            ...
            $errors[] = "Не удалось опубликовать объявление - не получены данные по этому объекту из базы. Повторите попытку немного позже или свяжитесь с нами: 8-922-179-59-07";
        }
...
if ($action == "unpublishAdvert" && $propertyId != "" && $propertyId != 0) {
    ...
    if ($collectionProperty->hasPropertyId($propertyId)) {
        ...
        if ($property->readCharacteristicFromDB()) {
            ...
            $errors[] = "Не удалось снять с публикации объявление - ошибка обращения к базе. Повторите попытку немного позже или свяжитесь с нами: 8-922-179-59-07";
        }
...
$userIncoming->searchProperties(20, $searchRequest->getTypeOfSorting());
