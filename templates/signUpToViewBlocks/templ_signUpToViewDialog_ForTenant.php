<form method="post" name="signUpToViewDialog" id="signUpToViewDialog"
      action="property.php?action=signUpToView<?php echo "&propertyId=" . $propertyCharacteristic['id'];?>"
        ...
        <tr>
            <td colspan="2">
                <textarea name="convenientTime" rows="3"
                          placeholder="Например: 15 декабря 15.00 - 17.00"
                          title="Рекомендуем указать несколько вариантов - это позволит нам быстрее договориться с собственником"
                          autofocus></textarea>
            </td>
        </tr>
        <tr>
