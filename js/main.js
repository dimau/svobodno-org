...
// При формировании страницы на сервере на нее (в конец страницы) добавляется информация об уже ранее загруженных фотографиях по данному пользователю (или объекту недвижимости). Если таковых нет, то передается пустой массив. Информация помещается в переменную temp и быстро может перекочевать в uploadedFoto
// На клиенте подставленная сервером JSON-строка декодируется и мы получаем uploadedFoto не в виде строки, а в виде массива объектов. Каждый объект содержит информацию по 1 фотографии
// На странице JS выполняет функцию createUploader для инициализации загрузки и редактирования фотографий
...
// При отправке заполненной формы на сервер registration.js (или другая страница) вызывает скрипт attrInputHiddenToValue в main.js, который заполняет поле value у INPUT hidden JSON-строкой, в которую преобразуется массив uploadedFoto.
// На сервере registration.php (или другая страница) разбирает эту строку и превращает ее снова в JSON-строку для передачи в браузер для JS (если возвращает некорректную форму клиенту), либо получая дополнительные данные из таблицы tempFotos сохраняет данные по фотографиям для постоянного хранения в таблицу userFotos в Базу данных на сервере
...
// Используется на странице регистрации (требуется при перезагрузке страницы, чтобы выбранные пользователем фотки не потерялись)
// Используется в личном кабинете при редактировании данных пользователя
// Используется на странице редактирования параметров объекта недвижимости, чтобы отобразить загруженнеы ранее пользователем фотографии
