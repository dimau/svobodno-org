...
function createUploadedFilesBlocks(uploader) {
    ...
    for (var i = 0; i < uploadedFoto.length; i++) {
        var uploadedFotoFolder = $(uploadedFoto[i]).attr('folder');
        var uploadedFotoId = $(uploadedFoto[i]).attr('fotoId');
        var uploadedFotoExtension = $(uploadedFoto[i]).attr('extension');
        var uploadedFotoName = $(uploadedFoto[i]).attr('filename');

        // Собираем объект с параметрами вызова функции _onComplete
        var rezult = {
            success: true,
            folder: uploadedFotoFolder,
            name: uploadedFotoId,
            ext: uploadedFotoExtension
        };
