    ...
    class qqFileUploader
    {
        ...
        function handleUpload($uploadDirectory, $replaceOldFile = FALSE)
        {
            ...
            function getSrcImage($srcParams, $tempFilePath)
            {
                ...
                switch (strtolower($srcParams['mime'])) {
                    ...
                    case "image/png":
                        ...
                        break;
                    default:
                        $foto_src = imagecreatefromgif("http://mobibase.ru/img/noimage.gif"); // TODO: картинка по умолчанию! здесь замени на свое чтонть.
                }
            ...
            function saveImg($foto_dst, $uploadDirectory, $type, $filename)
            {
                ...
                if (imagejpeg($foto_dst, $urlForSave)) {
            ...
            if (!$foto_dst) return array('error' => 'Не удалось сохранить загруженный файл на сервере.' . 'Загрузка была прервана, либо произошла ошибка на сервере.');
