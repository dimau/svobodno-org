<?php
    ...
    DBconnect::get()->query("CREATE TABLE searchRequests (
        ...
        interestingPropertysId TEXT COMMENT 'Список id объектов недвижимости, которыми заинтересовался пользователь в ходе поиска жилья. К просмотру анкет собственников данных объектов недвижимости пользователь имеет доступ в качестве потенциального арендатора'
)");
