<?php
     ...
     * $propertyLightArr - массив, содержащий минимум данных (координаты и id) по ВСЕМ объектам из БД, соответствующим поисковому запросу пользователя
     * $propertyFullArr - массив, содержащий полные данные по первым 20-ти (или другому числу) объектов
     * $userId - id пользователя, либо если он не авторизован - false
     **************************************************************************************************************/

    function getSearchResultHTML($propertyLightArr, $propertyFullArr, $userId = FALSE)
    {
        ...
        for ($i = 0; $i < count($propertyLightArr); $i++) {
            ...
            $rezPropertyFotos = mysql_query("SELECT id, extension FROM propertyFotos WHERE propertyId = '" . $propertyLightArr[$i]['id'] . "'");
            ...
            $matterOfBalloonList .= getFullBalloonHTML($propertyFullArr[$propertyLightArr[$i]['id']], $propertyFotosArr, $userId, $favoritesPropertysId);
            ...
            // Полученный HTML текст складываем в "копилочку"
            $matterOfShortList .= getShortListItemHTML($propertyFullArr[$propertyLightArr[$i]['id']], $propertyFotosArr, $userId, $favoritesPropertysId, $number);

            /***** Готовим блок fullParametersList таблицы для данного объекта недвижимости *****/
            // Полученный HTML текст складываем в "копилочку"
            $matterOfFullParametersList .= getFullParametersListItemHTML($propertyFullArr[$propertyLightArr[$i]['id']], $propertyFotosArr, $userId, $favoritesPropertysId, $number);
        }
    ...
    function getLightBalloonHTML($oneProperty) {
            // Шаблон для всплывающего баллуна без описания объекта недвижимости на карте Яндекса
            $tmpl_balloonContentBody = "<div class='balloonBlock' coordX='{coordX}' coordY='{coordY}' propertyId='{propertyId}'></div>";
            ...
            $arrBalloonTemplVar = array('{coordX}', '{coordY}', '{propertyId}');
    ...
    function getFullBalloonHTML($oneProperty, $propertyFotosArr = false, $userId = false, $favoritesPropertysId = false) {
        ...
        if ($userId != FALSE) {
            ...
            //TODO: что делать, если пользователь не авторизован - что ему выдавать, закодить здесь
        }

        // Производим заполнение шаблона баллуна
        // Инициализируем массив с строками, которые будут использоваться для подстановки в шаблоне баллуна
        $arrBalloonTemplVar = array('{propertyId}', '{typeOfObject}', '{address}', '{urlFoto1}', '{urlFoto2}', '{urlFoto3}', '{costOfRenting}', '{currency}', '{utilities}', '{compensationMoney}', '{compensationPercent}', '{amountOfRoomsName}', '{amountOfRooms}', '{adjacentRooms}', '{areaNames}', '{areaValues}', '{floorName}', '{floor}', '{furnitureName}', '{furniture}', '{urlProperty}', '{actionFavorites}', '{imgFavorites}', '{textFavorites}');
        // Копируем html-текст шаблона баллуна
        $currentAdvertBalloonList = str_replace($arrBalloonTemplVar, $arrBalloonReplace, $tmpl_balloonContentBody);
    ...
    function getShortListItemHTML($oneProperty, $propertyFotosArr = false, $userId = false, $favoritesPropertysId = false, $number) {
            ...
            <tr class='realtyObject' propertyId='{propertyId}'>
                <td>
            	    <div class='numberOfRealtyObject'>{number}</div>
               	    <span class='{actionFavorites}' propertyId='{propertyId}'><img src='{imgFavorites}'></span>
               	</td>
               	<td>
        ...
        // Инициализируем массив с строками, которые будут использоваться для подстановки в шаблоне баллуна
        $arrShortListTemplVar = array('{propertyId}', '{number}', '{actionFavorites}', '{imgFavorites}', '{urlFoto1}', '{address}', '{urlProperty}', '{costOfRenting}', '{currency}');
    ...
    function getFullParametersListItemHTML($oneProperty, $propertyFotosArr = false, $userId = false, $favoritesPropertysId = false, $number) {
        ...
        $tmpl_extendedAdvert = "
        <tr class='realtyObject' linkToDescription='{urlProperty}' propertyId='{propertyId}'>
            <td>
                <div class='numberOfRealtyObject'>{number}</div>
                <span class='{actionFavorites}' propertyId='{propertyId}'><img src='{imgFavorites}'></span>
            </td>
        ...
        // Инициализируем массив с строками, которые будут использоваться для подстановки в шаблоне баллуна
        $arrExtendedListTemplVar = array('{urlProperty}', '{propertyId}', '{number}', '{actionFavorites}', '{imgFavorites}', '{urlFoto1}', '{urlFoto2}', '{typeOfObject}', '{district}', '{address}', '{amountOfRooms}', '{adjacentRooms}', '{areaValues}', '{floor}', '{furniture}', '{costOfRenting}', '{utilities}', '{compensationMoney}', '{compensationPercent}');
        // Копируем html-текст шаблона блока (строки таблицы)
