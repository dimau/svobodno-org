...
function isAdvertCorrect($typeOfValidation)
{
    ...
    if ($dateOfCheckOut == "") $errors[] = 'Укажите крайний срок выезда для арендатора(ов)';
    if ($dateOfCheckOut != "") {
    ...
    if ($amountOfRooms == "0") $errors[] = 'Укажите количество комнат в квартире, доме';
    if ($adjacentRooms == "0") $errors[] = 'Укажите: есть ли смежные комнаты в сдаваемом объекте недвижимости';
    if ($amountOfAdjacentRooms == "0") $errors[] = 'Укажите количество смежных комнат';
    if ($typeOfBathrooms == "0") $errors[] = 'Укажите тип санузла';
    if ($typeOfBalcony == "0") $errors[] = 'Укажите: есть ли балкон, лоджия или эркер в сдаваемом объекте недвижимости';
    if ($balconyGlazed == "0") $errors[] = 'Укажите остекление балкона/лоджии';
    if ($roomSpace == "") $errors[] = 'Укажите площадь комнаты';
    if ($roomSpace != "") {
        if (!preg_match('/^\d{0,5}$/', $roomSpace)) $errors[] = 'Неправильный формат для площади комнаты: должно быть не более 5 цифр';
    }
